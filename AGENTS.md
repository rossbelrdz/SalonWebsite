...
> arquitectura, deploy, convenciones o seguridad. Última revisión completa: 2026-07-18 (v0.7.3).
...
└── web/                      # app Next.js
    ...
    └── src/
        ...
        │                     # servicios, sucursales, plataforma (superadmin)
        ├── app/empleado/     # agenda hoy/mañana + solicitud de ausencia
        ├── app/api/          # ~30 route handlers
        ├── components/       # AdminShell, PublicNav, NotificationBell, PwaRegister, ui.tsx
...
- Variables de entorno requeridas: ver `.env.example` raíz (infra Postgres/Redis/MinIO,
  ...
  **El directorio aún no es un repo git** — al inicializarlo, el `.gitignore` ya cubre
  `.env` y `docs/credentials.md`; verificar con `git status` antes del primer commit.
...
- Clases CSS del mockup como contrato (`.btn`, `.card`, `.badge`, `.sidebar`…); tema por
  tenant inyectado en `<style>` desde `app/layout.tsx` (`web/src/lib/theme.ts`).
- zod instalado pero **sin uso** — validación manual en handlers.

## 8. Seguridad — estado actual (auditoría 2026-07-18)
